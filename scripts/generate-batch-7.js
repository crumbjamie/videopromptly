...
const BATCH_7_CONFIG = {
  batchNumber: 7,
  priority: '4star-content',
  mode: 'fast', // $0.40 per video
  maxVideos: 23, // Remaining 4-star prompts
  logFile: `batch_generation_7.log`
};

// Batch 7 Prompt IDs - Remaining 4-Star prompts (23 total)
const BATCH_7_PROMPT_IDS = [
  // Remaining 4-Star prompts (23 of 31 total)
  425, 426, 428, 429, 430, 432, 433, 434, 435, 438, 439, 440, 441, 442, 443, 447, 448, 449, 451, 452, 453, 456, 457
];
...
function log(message, writeToFile = true) {
  ...
  if (writeToFile) {
    const logPath = path.join(__dirname, '..', BATCH_7_CONFIG.logFile);
...
function initLogFile() {
  ...
  const batchSize = Math.min(BATCH_7_PROMPT_IDS.length, BATCH_7_CONFIG.maxVideos);
  
  const header = `🎬 Starting batch generation for ${batchSize} remaining 4-star videos...

🎬 BATCH VIDEO GENERATION ${BATCH_7_CONFIG.batchNumber} - 4-STAR CONTENT COMPLETION
=======================================================
📋 Generating ${batchSize} videos (4-star completion focus)
⏰ Started: ${startTime}
...
async function generateBatch7() {
  log(`🎬 BATCH ${BATCH_7_CONFIG.batchNumber} VIDEO GENERATION WORKFLOW`);
  log('==================================================');
  ...
  const promptIds = BATCH_7_PROMPT_IDS.slice(0, BATCH_7_CONFIG.maxVideos);
  let successCount = 0;
  let failureCount = 0;
...
if (require.main === module) {
  initLogFile();
  generateBatch7().catch(error => {
    log(`❌ Batch generation failed: ${error.message}`);
    process.exit(1);
  });
}

module.exports = { generateBatch7, BATCH_7_PROMPT_IDS };